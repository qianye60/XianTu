...
export function generateJudgmentPrompt(): string {
  return `
## 天道演算系统 v5.0 - 2d6宿命序列判定

### 核心机制
每次场景开始时预生成18次2d6骰子结果作为"宿命序列"，按"我方→敌方→我方→敌方"顺序消耗，骰子结果不可修改。

### 判定流程
1. **确定需要判定**：战斗/探索/学识/潜行/鉴定/施法/侦查等行动
2. **提取骰子**：从宿命序列按顺序取出下一个2d6结果（如[3,5]=8）
3. **计算最终值**：基准值（属性+技能+装备+状态）+ 骰子结果
4. **比对目标值**：最终值 vs 目标难度值
5. **判定等级**：
   - 双一(1,1)：大失败（自动失败）
   - 双六(6,6)：大成功（自动成功+神之骰事件）
   - 最终值 ≥ 目标值+6：大成功
   - 最终值 ≥ 目标值+3：完全成功
   - 最终值 ≥ 目标值：成功
   - 最终值 ≥ 目标值-3：失败
...
### 战斗判定
**命中判定**：攻击方判定成功 → 进行伤害计算
**伤害公式**：最终伤害 = (攻击方属性+武器+技能) - (防御方韧性+护甲)
- 若伤害 ≤ 0，攻击无效
- 生命值归0才死亡

**战斗播报要求**：
- 必须明确显示：骰子结果、最终达成值、目标值、是否成功
- 命中后必须显示：原始伤害、护甲减免、最终伤害、剩余生命值
- 示例："你投出[4,5]=9，加上基准值8，最终达成17，目标值12，判定成功！造成15点伤害，敌人护甲减免5点，实际造成10点伤害。敌人剩余生命值：25/35"

### 敌人AI行为
根据剩余血量决定行动：
- 血量>50%：积极进攻
- 血量20%-50%：谨慎应对
- 血量10%-20%：尝试逃跑/濒死反扑/使用保命技能
- 血量归0：死亡
