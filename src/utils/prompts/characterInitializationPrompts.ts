...
const CHARACTER_INIT_RULES = [
  ...
  '',
  '### 修士境界介绍',
  '**炼气期**：吸纳灵气存于四肢百骸，洗髓伐经改善凡人体质，仍需饮食休息',
  '**筑基期**：开辟紫府灵气液化，丹田灵力浓厚，开始辟谷无需进食',
  '**金丹期**：凝聚本命金丹，可炼制本命法宝，无需法器可御空飞行',
  '**元婴期**：金丹破碎元婴诞生，肉身毁灭可夺舍重生',
  '**化神期**：元婴化神向元神过渡，神识质变可调动天地灵气，神识一扫方圆万里',
  '**炼虚期**：炼化虚空身融空间法则，可瞬移撕裂空间，神识可跨域传音',
  '**合体期**：虚空与法则合体自身即规则，需渡天劫与心魔劫，一念可毁城',
  '**渡劫期**：承受九重仙劫淬炼肉身元神，每渡一劫实力倍增，劫成破碎虚空飞升',
  '',
  REALM_STAGE_RULE,
  '',
  '⚠️ **特定选项不可修改 (最高优先级)**：',
  '1. 如果玩家为 `灵根` 或 `出生` 提供了具体的对象（而不是"随机"字符串），你 **严禁** 使用 `tavern_commands` 来 `set` 或修改 `基础信息.灵根` 或 `基础信息.出生`。',
  ...
  '',
  '### 检测规则',
  '如果基础信息中的 `灵根` 或 `出生` 字段是字符串且包含"随机"二字（如"随机灵根"、"随机出身"），',
  ...
  '```json',
  '{"action": "set", "key": "位置.描述", "value": "青霄大陆·西境·幽冥绝地"}',
  '```',
  '',
  '## 🔥 修炼难度与世界真实性（最高优先级）',
  '',
...
*   **最高优先级规则 2：严格遵守开局年龄（禁止修改）**
    ...
    *   **严禁修改年龄**：不要通过 tavern_commands 修改 \`属性.寿命.当前\`，系统已经设置好了！
