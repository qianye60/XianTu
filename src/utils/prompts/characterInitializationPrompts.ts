...
const CHARACTER_INIT_RULES = [
  ...
  '',
  '### 随机选项处理 (最高优先级！)',
  '🔴 **强制要求：必须使用 tavern_commands 替换所有随机选项**',
  '',
  '#### 随机灵根处理',
  '**检测条件**：如果角色的灵根字段值为"随机灵根"或包含"随机"关键字',
  '**必须执行**：',
  '1. 根据角色天资等级选择合适的灵根品级',
  '   - 废柴/凡人 → 凡品、下品灵根',
  '   - 俊杰 → 中品、上品灵根',
  ...
  '',
  '#### 随机出身处理',
  '**检测条件**：如果角色的出生字段值为"随机出身"或包含"随机"关键字',
  '**必须执行**：',
  '1. 根据世界背景创造一个独特的出身故事',
  '2. 出身必须与世界设定、角色性别、天资相匹配',
  '3. **必须使用以下命令替换角色基础信息中的出生字段**：',
  '```json',
  '{"action": "set", "key": "角色基础信息.出生", "value": {',
  '  "名称": "修仙世家传人",',
  '  "描述": "出生于传承千年的修仙世家，自幼接受系统修行训练..."',
  '}}',
  '```',
  '',
  '🚨 **严重警告**：',
  '- 如果检测到"随机灵根"或"随机出身"，但未使用 tavern_commands 替换，视为任务失败！',
  '- 替换命令必须放在 tavern_commands 数组的前面（在设置时间之后，创建NPC之前）',
  '- 不允许在最终输出中保留"随机灵根"或"随机出身"字符串！',
  '',
  '### 位置设置要求',
  ...
  '',
  '#### 🚨 强制要求：根据系统设置严格生成私密信息',
  '⚠️ 重要：本次为初始化对话，必须严格按照 systemSettings.nsfwMode 和 systemSettings.nsfwGenderFilter 设置来生成私密信息！',
  '',
  '**生成规则（必须严格遵守）**：',
  '',
  '1️⃣ **当 systemSettings.nsfwMode = true 且 systemSettings.nsfwGenderFilter = "female"**',
  '   → 所有女性NPC必须包含完整的"私密信息"字段',
  '   → 男性NPC不生成私密信息',
  '',
  '2️⃣ **当 systemSettings.nsfwMode = true 且 systemSettings.nsfwGenderFilter = "male"**',
  '   → 所有男性NPC必须包含完整的"私密信息"字段',
  '   → 女性NPC不生成私密信息',
  '',
  '3️⃣ **当 systemSettings.nsfwMode = true 且 systemSettings.nsfwGenderFilter = "all"**',
  '   → 所有NPC（无论性别）都必须包含完整的"私密信息"字段',
  '',
  '4️⃣ **当 systemSettings.nsfwMode = false**',
  '   → 所有NPC都不生成私密信息',
  '',
  '**私密信息必填字段清单（13个字段，缺一不可）**：',
  '',
  '  1. 是否为处女/处男 (boolean) - 根据角色背景合理设定',
  '  2. 身体部位 (array of objects) - 必须遵循 `dataStructureDefinitions.ts` 中定义的结构。',
  '     【结构铁律】: 必须是一个对象数组, 每个对象包含 "部位名称", "开发度", "敏感度", "特征描述", "特殊印记"。',
  '     【正确示例】: [{ "部位名称": "胸部", "开发度": 10, "敏感度": 30, "特征描述": "发育良好，白皙柔软", "特殊印记": "无" }]',
  '  3. 性格倾向 (string) - 描述性格特点，如"温柔顺从"、"冷艳矜持"、"活泼开放"等',
  '  4. 性取向 (string) - 如"异性恋"、"双性恋"、"泛性恋"等',
  '  5. 性癖好 (array) - 至少2-3个，如["喜欢被抚摸乳房","喜欢深度接吻","对颈部敏感"]',
  '  6. 性渴望程度 (number, 0-100) - 当前性欲强度，根据角色状态设定',
  '  7. 当前性状态 (string) - 详细描述当前身体状态，如"平静放松，身体未有特殊反应"',
  '  8. 体液分泌状态 (string) - 详细描述体液状态，如"正常干燥"、"微微湿润，有淡淡香气"',
  '  9. 性交总次数 (number) - 根据是否为处女/处男和背景故事设定',
  '  10. 性伴侣数量 (number) - 必须与性伴侣名单数组长度一致',
  '  11. 性伴侣名单 (array) - 字符串数组，处女/处男则为空数组[]',
  '  12. 最近一次性行为时间 (string) - 处女/处男则为"无"，否则描述具体时间',
  '  13. 特殊体质 (array) - 如["敏感体质","易高潮体质","媚骨天成"]，可为空数组',
  '',
  '**描述规范（必须遵守）**：',
  '- 身体部位描述必须详细、色情、富有想象力，专注于感官细节和触感描述',
  '- 使用修仙世界的文学化、唯美化的语言风格，如"肌肤凝脂如玉"、"媚骨天成"',
  '- 避免过于粗俗的用词，保持雅致的情色美感',
  '- 每个身体部位至少包含：外观描述、触感描述、敏感度描述',
  '',
  '**逻辑一致性（必须严格遵守）**：',
  '- 是否为处女/处男=true 时：性交总次数=0, 性伴侣数量=0, 性伴侣名单=[], 最近一次性行为时间="无"',
  '- 是否为处女/处男=false 时：以上字段必须符合逻辑，性伴侣数量必须等于性伴侣名单.length',
  '',
  '#### 玩家私密信息生成（可选，仅当systemSettings.nsfwMode=true时生成）',
  '- 如果 systemSettings.nsfwMode=true，为玩家生成完整的私密信息对象，包括3-5个核心身体部位。',
  '- 【重要】玩家的私密信息结构与NPC完全一致，但存储在 `玩家角色状态.私密信息` 路径下。',
  '- 使用命令格式：{"action":"set","key":"玩家角色状态.私密信息","value":{...私密信息完整对象...}}',
  '- 身体部位数组必须遵循对象结构：`[{ "部位名称": "...", "开发度": ..., ... }]`',
  '',
  '🚨 严重警告：',
  '- 未按照 systemSettings 设置生成私密信息将被视为严重错误！',
  '- 如果设置要求生成女性私密信息，但实际未生成，视为任务失败！',
  '- 必须检查 systemSettings.nsfwMode 和 systemSettings.nsfwGenderFilter 的值！',
].join('\n');
...
- 严格禁止：除了出生日期，不允许使用任何命令修改角色基础信息下的其他字段
- 严格禁止：不允许修改角色基础信息.名字、角色基础信息.性别、角色基础信息.先天六司等字段
- 严格禁止：玩家选择的天赋、灵根、出身等数据都是固定的，不能被修改
...
注意：非修仙世界的货币作为物品存储，使用 \`背包.物品.currency_xxx\` 路径
...
- 资产: \`背包\` ({灵石, 物品})
- 私密信息: 此次对话为初始化，必须生成，包含13个必填字段的完整对象
- 元信息: \`实时关注\` (boolean)
...
**当前系统配置（必须严格遵守）**：
- **nsfwMode**: ${worldContext?.systemSettings?.nsfwMode ? 'true（已启用）' : 'false（已禁用）'}
- **nsfwGenderFilter**: ${worldContext?.systemSettings?.nsfwGenderFilter || 'female'}

**生成规则（最高优先级）**：

${worldContext?.systemSettings?.nsfwMode === false ? `
❌ **NSFW模式已禁用**
→ 所有NPC都不生成私密信息
→ 不生成玩家身体部位开发
` : worldContext?.systemSettings?.nsfwGenderFilter === 'female' ? `
✅ **NSFW模式已启用 - 仅女性**
→ 所有女性NPC必须包含完整的14字段"私密信息"
→ 男性NPC不生成私密信息
→ 为玩家生成3-5个身体部位开发数据
` : worldContext?.systemSettings?.nsfwGenderFilter === 'male' ? `
✅ **NSFW模式已启用 - 仅男性**
→ 所有男性NPC必须包含完整的14字段"私密信息"
→ 女性NPC不生成私密信息
→ 为玩家生成3-5个身体部位开发数据
` : `
✅ **NSFW模式已启用 - 所有性别**
→ 所有NPC（无论性别）都必须包含完整的14字段"私密信息"
→ 为玩家生成2-4个身体部位开发数据
`}
