...
const CHARACTER_INIT_RULES = [
  ...
  '   → 所有NPC都不生成私密信息',
  '',
  '**私密信息必填字段清单（14个字段，缺一不可）**：',
  '',
  '  1. 是否为处女/处男 (boolean) - 根据角色背景合理设定',
  '  2. 身体部位 (array) - 必须包含至少5个核心部位的详细、色情的感官描述',
  '     示例：["乳房：丰满圆润，乳晕粉嫩，触感柔软弹性十足","阴蒂：敏感凸起，轻触即有反应","阴道：紧致温润，内壁柔软"]',
  ...
  '  14. 敏感部位 (array) - 至少3个，如["乳头","阴蒂","耳垂","颈部","大腿内侧"]',
  '',
  '**描述规范（必须遵守）**：',
  ...
  '',
  '#### 玩家身体部位开发（可选，仅当systemSettings.nsfwMode=true时生成）',
  '- 如果 systemSettings.nsfwMode=true，为玩家生成3-5个核心身体部位',
  '- 使用命令格式：{"action":"set","key":"身体部位开发.部位名称","value":{"描述":"详细色情描述","开发等级":0-20,"敏感度":20-50}}',
  '- 描述必须详细、色情且富有想象力',
  '',
  '🚨 严重警告：',
