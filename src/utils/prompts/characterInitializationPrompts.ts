...
\`\`\`json
{
  ...
  "tavern_commands": [
    {"action": "set", "key": "玩家角色状态.位置.描述", "value": "大陆名·地点名（⚠️ 大陆名和地点名都必须从输入数据的世界信息.大陆信息和世界信息.地点信息中选择真实存在的名称，严禁编造）"},
...
1. **大陆名称**：必须从输入数据的 \`世界信息.大陆信息\` 数组中选择一个真实的大陆名称
2. **地点名称**：必须从输入数据的 \`世界信息.地点信息\` 数组中选择一个真实的地点名称
3. **严禁编造**：严禁编造任何不存在于输入数据中的大陆名或地点名
4. **格式**："真实大陆名·真实地点名"（用一个·分隔）

**示例**（假设输入数据中有这些真实数据）：
- 如果 世界信息.大陆信息 = [{名称: "朝天大陆"}]，世界信息.地点信息 = [{名称: "青云门"}, {名称: "天枢城"}]
...
你的任务是为给定的大陆名称生成一个具体的、富有特色的地点名称。
...
\`\`\`json
{
  "location_name": "具体的地点名称（不包含大陆名）"
}
\`\`\`

**地点名称要求**：
1. **简洁明了**：2-4个汉字的**单一地点名称**
2. **富有特色**：能体现修仙世界的特点
3. **不包含大陆名**：只返回地点名称本身，不要包含大陆名称
4. **不包含层级**：只生成单一名称，不要使用"·"符号分隔多个层级
5. **多样化**：每次生成都应该有所不同，避免使用常见套路名称
6. **示例**（✅ 正确）：
   - "天枢城"
   - "青云峰"
   - "幽谷"
