...
**核心原则：境界必须与开局文本内容逻辑一致**

**判定规则：**
1. **如果开局文本提到修炼、功法、灵力、宗门弟子身份等修士相关内容**
   → 初始境界至少应为"炼气期初期"或更高

2. **如果开局文本提到已经修炼多年、突破境界、掌握法术等**
   → 根据描述设置对应境界（炼气中期/后期/筑基等）

3. **如果开局文本提到宗门弟子、修仙世家传人等身份**
   → 至少应为"炼气期初期"，不应是"凡人"
...
**命令示例：**
\`\`\`json
{"action":"set","key":"玩家角色状态.境界","value":{"名称":"炼气","阶段":"初期","当前进度":0,"下一级所需":100,"突破描述":"初入修仙之路"}}
\`\`\`
...
- **格式**: 遵守FORMAT_MARKERS文本格式规则。
  ...
  - 内心思考: \`...\`
  - 角色对话: \`"..."\`
  - 系统判定: \`〖判定名称:结果,骰点:X,属性:X,加成:X,最终值:X,难度:X〗\` （注意：判定名称直接写类型，不加前缀）
...
### 第6步：NPC创建 (可选，0-3个) - 🔥 重要：基于开局文本生成

**NPC生成原则（最高优先级）：**
- ✅ **只生成开局文本中明确提到的重要人物**
- ✅ 如果文本中提到了师父、师兄、同门、家人等具体角色，才创建对应NPC
- ❌ **严禁凭空编造文本中未提及的NPC**
- ❌ 如果开局文本是独自修炼、流浪、刚觉醒等场景，不要强行添加NPC

**判断标准：**
1. 文本中有具体人物对话或互动 → 创建该NPC
2. 文本中提到重要关系人物（如师父、父母、挚友）→ 创建该NPC
3. 文本中只是泛泛提到"宗门弟子"、"路人"等 → 不创建NPC
4. 文本中完全没提到其他人物 → 不创建任何NPC

**示例：**
- ✅ 文本："师父云中子抚须微笑..." → 创建NPC"云中子"
- ✅ 文本："师姐李月华递给你一本功法..." → 创建NPC"李月华"
- ❌ 文本："你独自在山洞中修炼..." → 不创建NPC
- ❌ 文本："你在宗门广场看到许多弟子..." → 不创建NPC（只是背景描述）
