...
### JSON结构示例：
 \`\`\`json
 {
   ...
   "tavern_commands": [
     ...
     {"action":"set","key":"角色.身体","value":{"身高":"<根据性别年龄>","体重":"<合理值>","体脂率":"<合理值>","三围":{"胸围":"<>","腰围":"<>","臀围":"<>"},"外观特征":["<根据角色特点>"],"肤色":"<>","发色":"<>","瞳色":"<>","纹身与印记":[],"穿刺":[],"部位":{},"部位开发":{}}},
     {"action":"set","key":"角色.属性.声望","value":"<根据出身0-100>"},
     {"action":"set","key":"角色.背包.灵石","value":{"下品":"<根据出身>","中品":0,"上品":0,"极品":0}}
   ],
...
3. **⚠️ 玩家法身（酒馆端必须执行！）**
   ...
   - **必填字段**（根据角色性别/年龄/种族合理生成）：
     ...
     - 外观特征: string[] - 根据角色特点描述
     - 肤色/发色/瞳色: string
     - 纹身与印记/穿刺: [] (空数组)
     - 部位/部位开发: {} (空对象)
   - **nsfwMode=true时额外生成**：胸部描述/私处描述/生殖器描述/敏感点
...
7. **NPC** - 仅创建文本中明确提到的重要人物（0-3个）
   - 写入 \`社交.关系.{NPC中文名}\`
   - **必填字段**：先天六司/外貌描述(50字+)/性格特征(3个+)/天赋/记忆(2条+)/属性
...
\`\`\`
{"action":"set","key":"角色.身体","value":{身高,体重,体脂率,三围:{胸围,腰围,臀围},外观特征:[...],肤色,发色,瞳色,纹身与印记:[],穿刺:[],部位:{},部位开发:{}}}
\`\`\`
