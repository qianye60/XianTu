...
### JSON结构示例：
 \`\`\`json
 {
   "text": "这里是1000-1500字的开局叙事正文...",
   "mid_term_memory": "这里是50-100字的摘要",
   "tavern_commands": [
     {"action":"set","key":"元数据.时间","value":{"年":1050,"月":1,"日":1,"小时":8,"分钟":0}},
     {"action":"set","key":"角色.身份.出生日期","value":{"年":1032,"月":1,"日":1}},
     {"action":"set","key":"角色.位置","value":{"描述":"东荒大陆·青云山脉·小村庄","x":5000,"y":5000,"灵气浓度":30}},
     {"action":"set","key":"角色.属性.声望","value":0},
     {"action":"set","key":"角色.身体","value":{"身高":170,"体重":60,"体脂率":18,"三围":{"胸围":88,"腰围":64,"臀围":90},"外观特征":["体态匀称","肌肤细腻"],"肤色":"偏白","发色":"乌黑","瞳色":"深褐","纹身与印记":[],"穿刺":[],"部位":{},"部位开发":{}}},
...
2. **位置** - 设置 \`角色.位置\`
   ...
   - \`灵气浓度\` 范围1-100，影响修炼速度（普通地点20-40，灵地50-70，洞天福地80+）
...
6. **NPC** - 仅创建文本中明确提到的重要人物（0-3个）
   ...
   - **必填字段质量要求**：
     - 先天六司:{根骨,灵性,悟性,气运,魅力,心性}各1-10,严禁全同,至少3项不同
     - 外貌描述:至少50字,含体态+面容+气质+穿着
     - 性格特征:至少3个,有表里层次
     - 天赋:至少1个{名称,描述}
     - 记忆:至少2条相关记忆
     - 属性:气血/灵气/神识必须与境界匹配
...
7. **大道** - 若天赋/功法影响大道
   - 写入 \`角色.大道.大道列表.{道名}\`
   - 必须包含完整对象：{道名,描述,阶段列表:[{名称,突破经验}],是否解锁:true,当前阶段:1,当前经验:0,总经验:0}

 8. **NSFW：玩家法身**（仅酒馆端）
   - 酒馆端必须初始化玩家 \`角色.身体\`（用于“法身”面板展示）
   - **无论 nsfwMode 开关**：至少要写入基础体格与外观字段（身高/体重/体脂率?/三围/外观特征/肤色/发色/瞳色/纹身与印记/穿刺）
   - **仅当 \`系统.配置.nsfwMode=true\`**：才允许/才生成敏感字段（胸部描述/私处描述/生殖器描述/敏感点/开发度）
   - 必须包含 \`部位:{}, 部位开发:{}\` 作为预留字段（用于变量面板/扩展系统写入）
   - 严禁使用“待AI生成/暂无/空对象”占位；数值要合理（单位：cm/kg/%）
...
- **法身数据**: 必须生成玩家 \`角色.身体\` 的基础体格数据（身高/体重/体脂率/三围/外观特征/肤色/发色/瞳色/纹身与印记/穿刺）
  ⚠️ 这是酒馆端必需的，用于"法身"面板展示，不要填"待AI生成"或留空！` : '- **运行环境**: 单机端（不需要生成法身数据）'}
