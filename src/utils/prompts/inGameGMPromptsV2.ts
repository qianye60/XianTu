...
export const buildInGameMessagePrompt = (saveData: SaveData, autoGenerateNpc?: boolean, minNpcCount?: number): string => {
  ...
  promptParts.push(
    ...
    '**修炼**: 时间+/灵气-/进度+/境界进度+',
    '',
    '**突破**: set境界(含名称/阶段/进度/所需/突破描述)+add寿命上限+add属性上限',
    '',
    ...
    '',
    '## 世界规则',
    '境界压制绝对(低境界无法胜高境界) | 灵根天定 | 功法优劣 | 越阶必败',
    '',
    '## 修炼代价',
    '时间:炼气升层1-3年,筑基数十年 | 资源:突破耗50-200灵石,败则尽失 | 风险:强突→走火/跌境/损寿 | 心魔:急功/杀戮必反噬',
    '',
    '## 战斗规则',
    '敌全力(不留手/不等待/最优战术) | 境界压制 | 战术合理 | 失误必死 | 死亡终局(无"恰好救命")',
    '',
    '## 🔴 死亡与受伤(核心玩法)',
    '',
    '**回档流游戏,死亡/受伤必须频繁发生**',
    ...
    '',
    '**触发**(满足之一)：越阶/重伤续战/入死地/怒强者/强突败/毒伤未治',
    '',
    '**禁止**：运气逃脱/敌失误/前辈救/低胜高/灵光化险',
    '',
    '## 玩家叙事权',
    '',
    '**可述**：自身行动/意图/选择',
    '',
    '**不可述(拒绝)**：事件结果/NPC反应/剧情发展/超范围内容',
    '',
    '**处理**：识别口胡→忽略不合理→按真实规则推',
    '',
    '例：玩家说"运气捡至宝"→拒,按概率无或有危险 | "NPC被打动"→拒,NPC按性格判断 | "灵光突破"→拒,需长期积累',
    '',
    '## 叙事风格',
