import { REALM_STAGE_RULE } from './sharedRules';
...
\`\`\`json
{
  "name": "出身名称",         // 名称: 根据用户提示词生成符合出生, 如"书香门第"（4-6字）
...
# 任务：生成先天天赋
## 要求：
1. 名称：符合修仙世界观，体现其先天特性（如：天生剑心、丹道圣体）。
2. 描述：一段简洁的文字，说明该天赋的本质，不涉及具体数值或后天效果。
