...
- 种族: string (默认"人族")
- 出生: string (出身背景)
- 年龄: number (系统自动计算得到；创建/更新时**允许省略**，禁止手动set/修改)
...
- 先天六司: {根骨: number, 灵性: number, 悟性: number, 气运: number, 魅力: number, 心性: number}
  - 取值范围：1-10（整数）
  - **严禁**六项全部相同（例如全是5）；必须有明显差异，并与NPC出身/性格/境界相匹配
  - 信息不足也必须生成“合理且有区分度”的六司，不要用模板默认值糊弄
- 属性: {气血: {当前, 上限}, 灵气: {当前, 上限}, 神识: {当前, 上限}, 寿元上限: number}
  - 气血/灵气/神识的当前值用add修改，上限在突破时用add
  - 寿元上限在突破时用add增加，当前年龄由出生日期自动计算
  - **数值必须与境界匹配**：参考 worldStandards 中的《人族境界属性标准》；筑基及以上禁止长期使用占位符默认值(气血100/灵气50/神识30)
...
- 私密信息?: PrivacyProfile (NSFW模式专用；当 系统.配置.nsfwMode=true 且 性别符合 系统.配置.nsfwGenderFilter 时为必填，禁止“待生成/暂无/空对象”)

**NPC创建强规则（必须遵守）**
- 创建NPC必须一次性 \`set 社交.关系.{NPC名}\` 写入完整对象（不要分步 set 子字段）
- 上述字段除“年龄/私密信息(可选)”外，创建时都必须提供；数组字段允许空数组但不能缺失（如 \`性格特征: []\`、\`人格底线: []\`、\`记忆: []\`、\`记忆总结: []\`）
...
仅NSFW模式可用；当 性别符合 系统.配置.nsfwGenderFilter 时必须生成完整对象：
- 是否为处女: boolean
...
- 小时: number (0-23)
- 分钟: number (0-59)
- **🔴 时间推进铁律**：必须使用 add \`元数据.时间.分钟\` 或 add \`元数据.时间.小时\` 推进，系统自动进位
  - ✅ 正确：\`元数据.时间.分钟\` / \`元数据.时间.小时\`
  - ❌ 错误：\`游戏时间.分钟\` / \`时间.分钟\` / \`系统.游戏时间.分钟\`
