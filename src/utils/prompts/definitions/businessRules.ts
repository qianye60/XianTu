...
阶段索引:当前阶段从1开始计数(1=入门,2=初窥...7=归一);阶段列表数组索引从0开始
`.trim();
...
[功法系统]
路径:功法数据=角色.背包.物品.{功法ID};修炼进度/已解锁技能=角色.背包.物品.{功法ID}.修炼进度/已解锁技能;当前修炼=角色.修炼.修炼功法
禁止:禁止 角色.修炼.功法进度.* (该路径已废弃不再使用)
装备流程:1.set当前功法为功法ID→2.set功法的修炼中=true,修炼进度=0,已解锁技能=[]→3.set之前功法修炼中为false
