/**
 * @fileoverview 游戏业务逻辑规则 v2.1 (精简版)
 */
...
## 境界与核心数值一致（CRITICAL）
- NPC 的核心战斗数值完全由 \`社交.关系.[NPC名].境界\` 决定：**高境界绝不允许低气血/低灵气/低神识**。
- 你必须参考 \`worldStandards\` 中的《人族境界属性标准 (Human Realm Stats Standard)》：那张表是“各境界大圆满参考值”。
- 当 NPC 境界发生变化（突破/降阶/重塑）时，**必须同回合同步更新**：
  - \`社交.关系.[NPC名].属性.气血.上限\` / \`当前\`
  - \`社交.关系.[NPC名].属性.灵气.上限\` / \`当前\`
  - \`社交.关系.[NPC名].属性.神识.上限\` / \`当前\`
  - \`社交.关系.[NPC名].属性.寿元上限\`
- 禁止出现“占位符数值”长期存在：
  - 除非境界=凡人，否则 \`气血上限=100\` / \`灵气上限=50\` / \`神识上限=30\` 这类默认值只允许作为**临时兜底**，不能用于筑基及以上。
- 阶段换算（经验规则）：在同一境界下，初期/中期/后期/圆满/极境 的上限应当分层递增（不要让“初期≈圆满”）。

## 战斗伤害与数值一致（关键后果）
- 伤害写入 \`社交.关系.[NPC名].属性.气血.当前\` 时必须满足：\`当前 ≥ 0\` 且 \`当前 ≤ 上限\`。
- “小技能一击秒杀同境界道侣”通常是数值错误：除非对方已濒死/有致命弱点/差大境界碾压，否则不要写入超过 60% 上限的单次伤害。
...
## 指令示例
{\"action\":\"add\",\"key\":\"角色.属性.灵气.当前\",\"value\":-15}
`.trim()
...
## 基准
- **基准单位**：下品灵石 = 1 价值度（所有价格优先用“价值度/下品灵石折算”表达）
- **默认币种价值度（锚点）**：
  - 下品灵石=1；中品=100；上品=10000；极品=1000000
  - 铜币=0.00001；银两=0.001；金锭=0.1

## 定价铁律（防乱价）
- 给出价格前，先判断：物品类型 + 品质(凡/黄/玄/地/天/仙/神) + 稀缺性(常见/稀有/绝版) + 地点(坊市/宗门/荒野)
- **同类同品质**的价格波动一般不超过 3x；超过必须写明原因（拍卖、垄断、战争封锁、黑市、断供等）
- 地区差异允许存在：可用“地区货币波动系数(0.6~1.6)”体现，但不要随口编出离谱汇率

## 常见价格锚点（以“下品灵石”为单位，粗略范围）
- 丹药：凡(0.1~1) / 黄(1~5) / 玄(5~20) / 地(20~80) / 天(80~300)
- 法宝/装备：凡(0.5~3) / 黄(3~15) / 玄(15~60) / 地(60~200) / 天(200~800)
- 功法/秘笈：凡(3~20) / 黄(20~80) / 玄(80~300) / 地(300~1200) / 天(1200~5000)
- 材料：按用途与稀缺性在 (0.05~200) 内取值；高阶材料必须匹配境界/秘境来源
`.trim()
...
- 类型：buff | debuff
- 状态描述：string
- 生成时间：{年,月,日,小时,分钟}（使用当前元数据.时间）
- 持续时间分钟：number

## 示例
{"action":"push","key":"角色.效果","value":{"状态名称":"灵气充盈","类型":"buff","状态描述":"灵气恢复速度提升50%","生成时间":{"年":1000,"月":3,"日":15,"小时":10,"分钟":30},"持续时间分钟":120}}
`.trim();

export const LOCATION_UPDATE_RULES = `
# 位置更新
触发：移动/场景变化时必须更新
格式：set 角色.位置 {描述:"大陆·区域·地点",x:5000,y:5000,灵气浓度:40}

## 字段说明
- 描述：格式"大陆·区域·地点"，必填
- x/y：坐标，中心(5000,5000)，东+x/西-x/北-y/南+y
- 灵气浓度：1-100，必填，影响修炼速度
  - 1-20：稀薄（凡间城镇、荒野）
  - 21-40：普通（普通修炼场所）
  - 41-60：充沛（宗门福地、灵脉附近）
  - 61-80：浓郁（秘境、洞府核心）
...
顶层：元数据. / 角色. / 社交. / 世界. / 系统.
- NPC\u5b57\u6bb5\u5fc5\u987b\u4ee5 \`\u793e\u4ea4.\u5173\u7cfb.[NPC\u540d].\` \u5f00\u5934\uff0c\u7981\u6b62\u5199\u6210 \`NPC\u540d.\u5b57\u6bb5\`
- NPC\u5185\u5fc3\u60f3\u6cd5/\u4fee\u5fc3\u72b6\u6001: set \`\u793e\u4ea4.\u5173\u7cfb.[NPC\u540d].\u5f53\u524d\u5185\u5fc3\u60f3\u6cd5\`
- NPC\u5bff\u5143\u4e0a\u9650: add/set \`\u793e\u4ea4.\u5173\u7cfb.[NPC\u540d].\u5c5e\u6027.\u5bff\u5143\u4e0a\u9650\`
...
## 时间路径（重要）
- 时间推进只允许写：元数据.时间.分钟 / 元数据.时间.小时
- 禁止使用：时间.分钟 / 游戏时间.分钟 / 系统.游戏时间.分钟
...
## 数据路径（V3）
- 功法物品：角色.背包.物品.{功法ID}（存储功法定义）
- 功法进度：角色.功法.功法进度.{功法ID}（熟练度/已解锁技能）
- 修炼引用：角色.修炼.修炼功法（当前正在修炼的功法引用）

## 学习新功法流程
1. set 功法物品到背包
2. set 功法进度初始化（熟练度0，已解锁技能[]）
3. set 修炼引用指向该功法

## 技能命名规范
格式：[意境]+[动作]+[诀/式/印]
禁止："技能1"、"火球术"等游戏化/西幻命名
`.trim();
...
## 灵气浓度影响（参考角色.位置.灵气浓度）
速度倍率：稀薄0.1-0.4 | 普通0.4-0.7 | 充沛0.7-1.0 | 浓郁1.0-1.5 | 极盛1.5-2.0

## 突破时间参考
练气期小阶段3月-3年，大境界2-20年；越高境界越久
`.trim();
...
# 宗门动态生成
延迟初始化：加入时只创建框架，藏经阁/贡献商店/宗门任务首次访问时生成
- 仅在“生成宗门藏经阁/贡献商店/宗门任务”这类明确的单次功能请求中使用本规则；正常剧情推进时不要凭空生成/覆盖这些内容
- 前置条件：只有当 \`社交.宗门 != null\` 且玩家已加入宗门时，才允许 set 写入 \`社交.宗门.宗门藏经阁.<宗门名>\` / \`社交.宗门.宗门贡献商店.<宗门名>\` / \`社交.宗门.宗门任务.<宗门名>\` / \`社交.宗门.宗门任务状态.<宗门名>\`
- \`<宗门名>\` 只是占位符示例；输出 key 必须写真实宗门名；禁止写 \`[宗门名]\` 这类占位符。方括号只能用于数组索引（如 \`[0]\`）
- 剧情提到某宗门/势力但玩家未加入：不要写 \`社交.宗门\`；应把该势力写入 \`世界.信息.势力信息\`
路径：社交.宗门.宗门藏经阁.<宗门名> | 社交.宗门.宗门贡献商店.<宗门名> | 社交.宗门.宗门任务.<宗门名> | 社交.宗门.宗门任务状态.<宗门名>
`.trim();
...
- 不要替玩家决定战斗中的关键选择
- 不要跳过交锋细节直接结算结果
- 单次战斗描写控制在约300-600字，并留出暂停点等待玩家反应
`.trim();