...
[🚨V3短路径强制规则]⚠️CRITICAL
所有tavern_commands的key必须以这5个顶级路径开头:元数据./角色./社交./世界./系统.
玩家角色数据:角色.xxx(如角色.位置/角色.属性.气血.当前)
NPC数据:社交.关系.{NPC名}.xxx(如社交.关系.李青云.好感度)
时间数据:元数据.时间.xxx
事件数据:社交.事件.事件记录(用push)

⛔错误路径示例(会被拦截!):
人物.张三.xxx ← 错!应为 社交.关系.张三.xxx 或 角色.xxx
事件.xxx ← 错!应为 社交.事件.事件记录
{玩家名}.xxx ← 错!玩家数据统一用 角色.xxx,不带玩家名
NPC名.xxx ← 错!必须 社交.关系.{NPC名}.xxx

[数据同步]⚠️
text写什么,tavern_commands就必须更新什么(否则删掉对应text内容)
约束:每类变化至少一条指令;新增NPC/信息必须完整落地到tavern_commands;禁止偷懒省略指令
若无法核对存档数据(名称/ID/路径不确定),宁可不写指令并删除对应的叙事变化;严禁猜测/编造
系统会拒绝无效/违规指令(不执行),所以请一次性给出可执行且完整的指令
...
□ 大道升阶 → add 角色.大道.大道列表.{道名}.当前阶段 (value:1,每次突破+1)
...
阶段突破:{"action":"add","key":"角色.大道.大道列表.剑道.当前阶段","value":1}
...
□ 顶级根:角色/社交/元数据/世界/系统
□ 角色子结构:角色.属性/角色.属性.气血/角色.属性.灵气/角色.属性.神识/角色.属性.寿命
□ 角色背包:角色.背包/角色.背包.物品/角色.背包.货币
□ 角色修炼:角色.功法/角色.功法.功法进度/角色.大道/角色.大道.大道列表/角色.效果/角色.技能
□ 社交子结构:社交.关系/社交.关系矩阵/社交.关系矩阵.edges/社交.事件/社交.事件.事件记录
□ 元数据:元数据.游戏设置
□ 世界:世界.地图/世界.势力

✅ 允许整体set的安全对象字段(常用且必须可改):
□ 位置变化 → set 角色.位置 {描述,x,y,地图ID?}
□ 时间跳跃/修复 → set 元数据.时间 {年,月,日,小时,分钟}（或对元数据.时间.xxx 用 add/set）
□ 大突破/重修 → set 角色.属性.境界 {名称,阶段,当前进度,下一级所需,突破描述}
...
重要:数值字段的value必须是JSON数字,禁止写成字符串("10")
