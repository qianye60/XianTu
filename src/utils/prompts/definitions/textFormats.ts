...
- 禁止反引号滥用：仅允许内心思考使用成对的单反引号 \`...\`（仅限NPC内心）；禁止三反引号/代码块；禁止未成对的反引号
...
\`\`\`
〖类型:结果,判定值:45,难度:50,基础:38,幸运:+2,环境:+5,状态:0〗
\`\`\`

**⚠️ REMEMBER / 记住：〖〗 NOT 【】 / 用〖〗不是【】**

### 约束（CRITICAL）
- 字段之间必须用英文逗号 \`,\` 分隔（推荐），不要写成“(基础:0 + 幸运:2 …) = 结果：…”这种括号/等号表达
- 冒号必须使用 \`:\` 或 \`：\`（推荐使用 \`:\`），且每个字段必须是 \`键:值\`
- 禁止占位符：不得出现 X/Y/???/“？？？”等；必须给出真实数值（无法给出就不要输出该判定行）
- 禁止把“系统提示/系统判定/【当前状态】”当作普通行输出：只能放进 〖...〗
...
❌ 〖战斗判定:成功〗 ← 缺少数值字段！
❌ (判定值45,难度35) ← 格式完全错误！
❌ 〖判定值:12 (基础:0 + 幸运:2) = 结果：？？？〗 ← 禁止括号/等号表达与占位符！
