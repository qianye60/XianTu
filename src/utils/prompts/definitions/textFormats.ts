...
### 幸运点（前端计算）
- **前端已计算**：系统会在"本回合判定数据"中提供计算好的幸运点
- **直接使用**：判定时直接使用提供的"幸运点"值，**不要自己计算**
- 幸运点基于角色气运属性计算，波动范围约 -10 到 +15
- 气运越高，幸运点的期望值和上限越高
...
✅ **完整版（推荐）**：
〖炼丹:成功,判定值:52,难度:50,基础:42(悟性21+境界12+技艺9),幸运:+8,环境:+2(灵气70),状态:0〗

〖斗法:失败,判定值:38,难度:45,基础:50(根骨25+境界12+技艺13),幸运:-2,环境:0,状态:-10(重伤)〗

〖突破:大成功,判定值:78,难度:60,基础:55(悟性28+境界20+技艺7),幸运:+15,环境:+3(灵气80),状态:+5(闭关)〗

〖感知:成功,判定值:92,难度:60,基础:72(灵性36+境界20+技艺16),幸运:+15,环境:+5(雷雨夜),状态:0〗
...
❌ **错误2 - 判定值组成不明**：
〖炼丹:成功,判定值:52,难度:50,幸运:+8〗  ← 52从哪来？缺少基础、环境、状态

❌ **错误3 - 基础值不透明**：
〖突破:成功,判定值:78,难度:60,基础:55,幸运:+15,环境:+3,状态:+5〗  ← 55怎么算的？
...
**输出格式**：
〖炼丹:成功,判定值:53,难度:50,基础:44(悟性21+境界14+技艺9),幸运:+8,环境:+1(灵气70),状态:0〗
`.trim()
