...
验算：6+2+11+3+12+4+4+16 = 58 ✓

### 核心规则
1. 难度必须带等级：\`30(普通)\` \`50(困难)\`
2. 加成必须注明来源：\`7(魅力×0.5+...)\` \`12(筑基初期)\`
3. **⚠️ 验算铁律**：先天+后天+境界+装备+功法+状态+其他+骰点 = 判定值（例：6+2+12+0+0+0+0+2=22）
4. **功法相关性**：社交/探索判定功法通常为0，战斗判定只有战斗功法有加成，修炼判定只有修炼功法有加成
...
**重要：功法加成只在相关判定中生效**
- **修炼判定**：修炼功法有加成
- **战斗判定**：战斗功法（剑诀、刀法等）有加成
- **技艺判定**：对应功法（丹诀、器诀等）有加成
- **社交判定**：功法无加成（除非是特殊的魅惑类功法）
- **探索判定**：功法无加成（除非是特殊的探查类功法）
