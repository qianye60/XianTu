...
4. **🔴 验算强制（违反立即重算）**：
   - 🔴🔴🔴 **必须逐项相加验算**：先天+后天+境界+[所有其他加成]+骰点 = 判定值
   - 🔴🔴🔴 **验算格式**：写出完整算式，如 "10+0+5+10+5+10+8+19 = 67"
   - 🔴🔴🔴 **验算不通过必须重算**：如果算式结果≠判定值，必须找出错误并重新计算
   - 每个加成都必须有明确来源
   - 不允许凭空增加数值
   - 🔴🔴🔴 **加法必须正确**：使用基础算术，逐步累加，不要跳步
...
**示例1（战斗判定）：**
先天:6(根骨10×0.5+...) + 后天:0 + 境界:11(炼气圆满) + 法宝:10(飞剑) + 功法:55(万道归一诀) + 骰点:16
🔴 验算（必须）: 6+0+11+10+55+16 = 6+0=6, 6+11=17, 17+10=27, 27+55=82, 82+16=98 ✓

**示例2（修炼判定）：**
先天:6(悟性11×0.5+...) + 后天:0 + 境界:5(炼气初期) + 阵法:3(简易警戒阵) + 丹药:5(聚灵丹) + 骰点:12
🔴 验算（必须）: 6+0+5+3+5+12 = 6+0=6, 6+5=11, 11+3=14, 14+5=19, 19+12=31 ✓
...
🔴🔴🔴 **比较规则**：
- 必须正确比较大小：77 < 80 是失败，77 > 80 是成功
- 不要搞混大于和小于的关系

### 4.2 CoT验算与检查

**验算要求（必须执行）：**
🔴🔴🔴 在CoT中必须逐步验算所有加成是否正确相加
🔴🔴🔴 验算格式（逐步累加）：
- 列出所有项：先天+后天+境界+[所有其他加成]+骰点
- 逐步计算：A+B=X, X+C=Y, Y+D=Z, ...
- 最终验证：Z = 判定值 ✓
🔴🔴🔴 如果验算不通过，必须找出错误并重新计算所有数值
