...
情绪疏导：神化主角→协作共赢 | NPC绝望→释怀接受 | 极端→平和过渡
注意：避免机械化/八股化/过度戏剧/过度反应/情感滥用
...
### 质量红线
[情绪疏导] AI易生成极端情绪，需转化而非压制：
  - 神化主角 → 转为协作共赢（NPC有独立价值，互相成就）
  - 绝望崩溃 → 转为释怀接受（承认困境，寻找出路）
  - 过度戏剧 → 转为平和过渡（情绪匹配刺激强度）
[正确] 角色独立人格/情绪匹配刺激/真实细节/非极端化
[错误] 八股化/机械化/过度反应/情感滥用
...
### 字数约束
每个思维链步骤标题后标注[约10-30字]，要求该步骤的思考内容控制在约10-30字左右，保持简洁高效
`;
}
