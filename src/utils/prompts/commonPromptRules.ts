...
1.  **精细化**: 如果玩家选择了"随机"出身或"随机"灵根，你必须将其具体化为一个合理的设定。
...
4.  **数据验证和类型检查**:
    *   **数字字段验证**: 年龄(如25)、境界等级(如5)、好感度(如80)必须是纯数字
    *   **境界对象验证**: 境界必须是完整对象 {等级: number, 名称: string, 当前进度: number, 下一级所需: number, 突破描述: string}，严格按照TypeScript接口定义
    *   **修为进度验证**: 修为进度存储在独立的修为字段 {当前: number, 最大: number}
    *   **字符串字段验证**: 行为模式必须是详细描述，不能是简单词语
    *   **对象字段验证**: 灵根必须包含名称、品级、描述三个字段
    *   **数组字段验证**: 背包.物品必须是数组[]，即使为空
    *   **格式检查**: 所有数据必须能被前端组件正确解析显示
