...
- 状态效果: [{状态名称, 类型, 剩余时间, 效果}] (⚠️ 只读！只能用push添加，禁止删除。系统会自动删除过期的状态效果)
...
- {NPC名字: NPC档案对象}
  ...
  - 背包: {灵石, 物品}
  - 私密信息: {12个必填字段} (【条件生成】仅当系统.nsfwMode=true且性别符合nsfwGenderFilter时生成)
    - 【必填字段】: 是否为处女/处男, 身体部位[], 性格倾向, 性取向, 性癖好[], 性渴望程度, 当前性状态, 体液分泌状态, 性交总次数, 性伴侣名单[], 最近一次性行为时间, 特殊体质[]
...
⚠️ **【最高优先级铁律】AI严禁通过tavern_commands修改游戏记忆字段** ⚠️

**严禁操作：**
- ❌ 严禁使用 \`set\` 操作修改 \`记忆.短期记忆\`
- ❌ 严禁使用 \`set\` 操作修改 \`记忆.中期记忆\`
- ❌ 严禁使用 \`set\` 操作修改 \`记忆.长期记忆\`
- ❌ 严禁使用 \`push\` 操作添加到任何记忆数组
- ❌ 严禁使用 \`delete\` 操作删除记忆字段
- ❌ 严禁使用任何命令清空记忆数组
...
**❌ 错误示例:**
- \`〖系统提示：星屑吊坠效果触发。〗\` (使用了中文冒号)
- \`〖修炼判定:成功〗\` (缺少骰点、属性等必要字段)
