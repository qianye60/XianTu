...
**严格格式要求**：\`〖判定名称:结果,骰点:X,属性:X,加成:X,最终值:X,难度:X〗\`
- 判定名称和结果之间**只能有一个冒号**
- 所有冒号必须使用英文冒号 \`:\`
- **错误示例**: \`〖系统判定:灵气感知:成功,...〗\` ❌（多了一层"系统判定:"）
- **正确示例**: \`〖灵气感知:成功,...〗\` ✅
...
判定格式 (必须包含所有字段，注意只有一个冒号):
- \`〖判定名称:结果,骰点:数值,属性名:数值,加成:数值,最终值:数值,难度:数值〗\`
- **判定名称**：直接写具体判定类型（如"灵气感知"、"炼器判定"、"剑术对决"），不要加"系统判定:"前缀
- **结果**：成功/失败/大成功/完美/大失败
- 骰点必须使用系统提供的值

正确示例（✅ 只有一个冒号）:
- \`〖修炼判定:成功,骰点:15,灵性:8,加成:5,最终值:28,难度:25〗\`
- \`〖炼器判定:失败,骰点:3,灵性:5,加成:2,最终值:10,难度:30〗\`
- \`〖灵气感知:成功,骰点:18,灵性:0,加成:5,最终值:23,难度:20〗\`
- \`〖战斗判定:大成功,骰点:18,根骨:9,加成:8,最终值:35,难度:20〗\`
- \`〖突破判定:完美,骰点:20,悟性:10,加成:15,最终值:45,难度:40〗\`
- \`〖炼丹判定:大失败,骰点:1,灵性:6,加成:3,最终值:10,难度:35〗\`

错误示例（❌ 多个冒号）:
- \`〖系统判定:灵气感知:成功,...〗\` ❌ 不要加"系统判定:"前缀
- \`〖判定:炼器判定:失败,...〗\` ❌ 不要加"判定:"前缀
- \`〖类型:修炼判定:成功,...〗\` ❌ 不要加任何多余前缀
