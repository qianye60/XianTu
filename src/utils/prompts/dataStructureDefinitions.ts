...
| 玩家角色状态.境界 | set | 🔴境界突破必须更新完整对象 | \`{"action":"set","key":"玩家角色状态.境界","value":{"名称":"练气","阶段":"初期","当前进度":0,"下一级所需":500,"突破描述":"感悟天地灵气，成功引气入体"}}\` |
| 玩家角色状态.声望 | add,set | 增减声望值 | \`{"action":"add","key":"玩家角色状态.声望","value":50}\` |
...
- **🔴 境界突破（最重要，经常遗漏）**: 必须更新 境界对象（名称+阶段+进度+所需+突破描述）+ 寿命上限 + 属性上限
  - **境界对象是最重要的**，属性变化只是结果，境界变化才是核心！
  - 突破描述 必须根据当前剧情动态生成，例如："借助筑基丹冲破瓶颈"、"感悟剑道真意破境"
  - **错误示例**: 只更新属性上限，不更新境界对象 ❌
  - **正确示例**: 先更新境界对象，再更新属性上限 ✅

**🔴 境界突破检查清单（必须全部完成）**：
1. ✅ 更新境界名称（凡人→练气、练气→筑基等）
2. ✅ 更新境界阶段（初期、中期、后期、圆满）
3. ✅ 重置当前进度为0
4. ✅ 设置下一级所需进度
5. ✅ 生成突破描述（结合当前剧情）
6. ✅ 更新寿命上限
...
- 感知度/唤醒度: 0 ~ 100
- 先天六司: 1 ~ 10
- 声望: 通常0-10000，可以为负数（恶名）
- add或set操作前必须计算，确保不超范围

## 4. 声望修改指导（重要）
**声望是玩家在修仙世界中的名声和地位，AI应该根据玩家行为主动调整：**

### 增加声望的情况（正面行为）：
- 击败强敌或恶人：+10到+100（根据敌人实力和影响力）
- 救助无辜民众：+5到+50
- 完成重要任务或委托：+20到+200
- 在宗门中立功：+30到+150
- 炼制出珍贵丹药并分享：+50到+300
- 创造新的功法或技能：+100到+500
- 击退魔族入侵等大事件：+200到+1000
...
**标准判定格式：**
- \`〖修炼判定:成功,骰点:45,灵性:8,加成:12,最终值:65,难度:50〗\`
- \`〖战斗判定:失败,骰点:23,根骨:6,加成:5,最终值:34,难度:40〗\`

**简单系统提示格式：**
- \`〖系统提示:星屑吊坠效果触发，悟性+2，灵性+2，凝神静气效果生效〗\`
- \`〖物品效果:获得筑基丹×3，灵石+500〗\`

**错误示例（禁止使用）：**
- ❌ \`〖系统提示：星屑吊坠效果触发，悟性+2，灵性+2，凝神静气效果生效。〗\` （缺少冒号分隔）
- ❌ \`〖星屑吊坠效果触发〗\` （没有明确的判定类型）
...
| **金丹** | 800 | 5,000 | 25,000 | 8,000 |
| **元婴** | 2,000 | 20,000 | 150,000 | 50,000 |
| **化神** | 5,000 | 100,000 | 1,000,000 | 300,000 |
| **炼虚** | 10,000+ | 500,000 | 8,000,000 | 2,000,000 |
| **合体** | 与世同君 | 2,500,000 | 50,000,000 | 15,000,000 |
| **渡劫** | 不定(劫数) | 此界极限 | 此界极限 | 此界极限 |
...
- 天：高级
- 仙: 顶级
- 神: 不可得之物，几乎不能出现在游戏中的顶级物品

## NPC物品配置
- 必须0-3件物品
- 品质匹配境界：炼气(凡)、筑基(黄)、化神(地)、...
`.trim()
