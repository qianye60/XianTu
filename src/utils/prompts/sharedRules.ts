...
2. **不合理要求必须判定失败**：境界不足、资源不足、技能不足、违背规则时必须判定失败。
...
**判定格式**（严格遵守，只有一个冒号）：\`〖判定名称:结果,骰点:X,属性:X,加成:X,最终值:X,难度:X〗\`
- **判定名称**：直接写具体判定类型（如"灵气感知"、"炼器判定"），不要加"系统判定:"等前缀
- 骰点必须使用系统提供的值
- 正确示例：\`〖炼器判定:失败,骰点:3,灵性:5,加成:2,最终值:10,难度:30〗\`
- 正确示例：\`〖灵气感知:成功,骰点:18,灵性:0,加成:5,最终值:23,难度:20〗\`
- 错误示例：\`〖系统判定:灵气感知:成功,...〗\` ❌
`.trim();
