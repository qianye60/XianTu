...
export async function initializeCharacterOffline(
  ...
  try {
    ...
    (saveData.玩家角色状态 as any)._AI说明 = '玩家实时状态。位置仅更新"描述"字段；气血/灵气/神识/寿命/修为字段为{当前,最大}结构，任何数值变更必须通过 tavern_commands 实现。';
    (saveData.玩家角色状态 as any)._AI重要提醒 = '⚠️ 严禁在位置中添加经度、纬度等字段，只能有"描述"字段；当气血≤0应设置已死亡/死亡时间/死亡原因';
